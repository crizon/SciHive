 ...
 */
var colorLuminance = function colorLuminance(color, alpha) {
	var toRGBA = function toRGBA(rgb_, a_) {
		return rgb_.replace("rgb", "rgba").replace(")", ","+a_+")");
	};

	var hex = new RegExp("#").test(color);
	var rgb_ = "";
	if (hex) {
		hex = color.replace("#", "");
...
var setParameters = function setParameters(params) {
  ...
  if (params.titleText) {
    ...
    title.style.color = params.color;
  } else {
  ...
  if (params.text || params.html) {
    ...
    content.style.color = params.color;
    show(content);
  } else {
    hide(content);
  }
  ...
  return new Promise(function (resolve, reject) {
    ...
    var onButtonEvent = function onButtonEvent(event) {
      ...
      switch (e.type) {
        ...
        case 'mouseup':
          if (params.buttonsStyling) {
            if (targetedConfirm && params.confirmButtonColor != "transparent") {
              confirmButton.style.backgroundColor = colorLuminance(params.confirmButtonColor, 0.8);
            } else if (targetedCancel && params.cancelButtonColor != "transparent") {
              cancelButton.style.backgroundColor = colorLuminance(params.cancelButtonColor, 0.8);
            }
          }
          break;
        case 'mouseout':
